...
export const DesktopHeaderMenu = ({
    ...
    suggestions,
    selectSuggestion,
}: TDesktopHeaderMenuProps) => {
    const { isExchangesPage, isCreatePage, isProfilePage, onCreate, onExchanges, onProfile } =
        useDesktopHeaderActions();
    const getActionClassName = (isActive: boolean) =>
        [Styles['header__action'], isActive && Styles['header__action--active']]
            .filter(Boolean)
            .join(' ');
    ...
    return (
        <div className={Styles['header']}>
            ...
            <Button
                variant={'default'}
                icon={
                    <span className={Styles['header__plus-icon']}>
                        <PlusSVG />
                    </span>
                }
