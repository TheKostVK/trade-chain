...
export const useCustomerRecommendationsEditor = () => {
    ...
    const selectedCategories = recommendations
        .map((recommendation) =>
            categories.find((category) => category.category_id === recommendation.category_id),
        )
        ...
        .filter(
            (category) =>
                !recommendations.some(
                    (recommendation) => recommendation.category_id === category.category_id,
                ),
        )
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            dispatch({
                ...
                value: parseApiError(
                    error,
                    'Не удалось обновить список интересов. Попробуйте ещё раз.',
                ),
            });
        }
    };
